...
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
    ...
    public boolean onTouch(View v, MotionEvent ev) {
       ...
       if(mDetector.onTouchEvent(ev)) {
        ...
        if (mRemoveEnabled && mDragging && mRemoveMode == FLING_REMOVE) {
            if(mFlingRemoveDetector.onTouchEvent(ev)) {
            	return true;
            }
        }
    ...
    public boolean onDown(MotionEvent ev) {
        ...
        if (mHitPos != MISS && mDragInitMode == ON_DOWN) {
            b = startDrag(mHitPos, (int) ev.getX() - mItemX, (int) ev.getY() - mItemY - 20);
        }
        ...
        if (mCanDrag && !mDragging && (mHitPos != MISS || mFlingHitPos != MISS)) {
            if (mHitPos != MISS) {
                if (mDragInitMode == ON_DRAG && Math.abs(y2 - y1) > mTouchSlop && mSortEnabled) {
                    b = startDrag(mHitPos, deltaX, deltaY);
                }
                else if (mDragInitMode != ON_DOWN && Math.abs(x2 - x1) > mTouchSlop && mRemoveEnabled)
                {
                    mIsRemoving = true;
                   b =  startDrag(mFlingHitPos, deltaX, deltaY);
                }
                    ...
                    if (mRemoveEnabled && mIsRemoving) {
                        ...
                        if (velocityX > mFlingSpeed) {
                            if (mPositionX > -minPos) {
                               b = mDslv.stopDragWithVelocity(true, velocityX);
                            }
