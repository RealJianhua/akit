...
package wenjh.akit.common.view.dragsort;
...
public class DragSortListView extends ListView {
    ...
    // 控制是否允许拖动
    private CanDropListener mCanDropListener;
    ...
    private boolean updatePositions() {
        ...
        if(mCanDropListener != null && !mCanDropListener.canDrop(itemPos-numHeaders)) {
        	updated = false;
        } else {
    ...
    private void adjustItem(int position, View v, boolean invalidChildHeight) {
        ...
        if (position == mFirstExpPos || position == mSecondExpPos) {
            if (position < mSrcPos) {
            	if(mCanDropListener != null && !mCanDropListener.canDrop(position-getHeaderViewsCount())) {
    ...
    public boolean startDrag(int position, View floatView, int dragFlags, int deltaX, int deltaY) {
        ...
        // 不允许拖动
        CanDropListener canDropListener = getCanDropListener();
        if(canDropListener != null) {
        	if(!canDropListener.canDrop(position)) {
        		return false;
        	}
        }
    ...
    public void setCanDropListener(CanDropListener l) {
		this.mCanDropListener = l;
	}

    public CanDropListener getCanDropListener() {
		return mCanDropListener;
	}
    ...
    public interface CanDropListener {
