...
public class ImageTricks {
    ...
    public static void scaleDownImageFile(File originalImageFile, int maxDimension, CompressFormat format, int quality) {
    	Bitmap b = ImageUtil.decodeFile(originalImageFile.getAbsolutePath());
    	if (b == null)
    ...
    public static Bitmap scaleDownImageUriToBitmap(Uri imageUri, int maxDimension, boolean deleteOriginal) {
    	try {
    		InputStream mediaStream = ContextUtil.getContentResolver().openInputStream(imageUri);
        	...
        	mediaStream = ContextUtil.getContentResolver().openInputStream(imageUri);
...
//        	bitmap = scaleDownBitmap(bitmap, maxDimension, true);
        	...
        	if (deleteOriginal)
        		ContextUtil.getContentResolver().delete(imageUri, null, null);
        	return bitmap;
    ...
    public static Uri scaleDownImageUri(Uri imageUri, int maxDimension, CompressFormat format, int quality, boolean deleteOriginal) {
    	try {
	    	...
	    	Uri rtr = Uri.parse(Images.Media.insertImage(ContextUtil.getContentResolver(), tmpFile.getAbsolutePath(), null, null));
	    	tmpFile.delete();
	    	return rtr;
    	} catch (Throwable e) {
    		throw new RuntimeException(e);
...
//    }
    ...
    public static Uri copyImageFileIntoGalleryAndGetUri(Context c, File imageFile, boolean deleteImageFileAfter) {
        if (imageFile.exists() && imageFile.isFile()) {
            try {
                Uri dataUri = Uri.parse(Images.Media.insertImage(c.getContentResolver(), imageFile.getAbsolutePath(), null, null));
                if (dataUri != null) {
    ...
    public static Uri putBitmapIntoGalleryAndGetUri(Context c, Bitmap image, boolean recycleOriginal) {
        if (image != null) {
            Uri dataUri = Uri.parse(Images.Media.insertImage(c.getContentResolver(), image, null, null));
            if (dataUri != null) {
            	ContentValues cv = new ContentValues();
            	cv.put(Images.Media.DATE_TAKEN, System.currentTimeMillis());
            	cv.put(Images.Media.DATE_ADDED, System.currentTimeMillis());
    ...
    public static Uri putBitmapIntoGalleryAndGetUri(Context c, File imageFile) {
    	 if (imageFile.exists() && imageFile.isFile()) {
             try {
            	 ...
            	 return c.getContentResolver().insert(IMAGE_URI, values);  
             } catch (Exception e) {
                 e.printStackTrace();
             }   
         }
