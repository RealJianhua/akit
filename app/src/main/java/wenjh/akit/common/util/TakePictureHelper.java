...
import wenjh.akit.demo.img.crop.CropImageActivity;
...
public class TakePictureHelper {
	...
	public void takeNewPictureFromCamera() {
		try {
			mCameraTempFile = new File(StorageConfigs.getImageCacheDir(), System.currentTimeMillis()+"");
	...
	private void compressImageAndCallback(Uri imageUri) {
		File storeFile = StorageConfigs.getImageCacheFile(System.currentTimeMillis()+"");
	...
	private void gotoCropImage(Uri uri) {
		mCropTempFile = new File(StorageConfigs.getImageCacheDir(), System.currentTimeMillis()+"");
